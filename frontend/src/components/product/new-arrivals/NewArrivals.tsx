...
export default function NewArrivals() {
  return (
    <div className="w-full max-w-7xl mx-auto py-4 mt-16">
      <div className="flex items-center space-x-2 mb-4">
        <h2 className="px-2 text-2xl font-semibold">New Arrivals</h2>
        <span className="inline-block px-4 py-1 rounded-full bg-primary text-white font-bold">
          NEW
        </span>
      </div>
          ...
          <div className="animate-marquee flex items-center">
            {newArrivals.map((product) => (
              <div key={product.id} className="flex-shrink-0 mx-6 group">
                <div className="relative h-full w-64 bg-gray-200 rounded-lg shadow-sm p-2 overflow-hidden flex flex-col justify-center transition-all duration-300 group-hover:shadow-md">
                  <Image
                    src={product.image}
                    alt={product.title}
                    width={120}
